...
const Member = (props) => {
  ...
  const [isOpenFormRemoveMember, setIsOpenFormRemoveMember] = useState({isOpenFormRemoveMember: true})
  ...
  useEffect(() => {
    if (props.isCloseFormRemoveMember) {
      setIsOpenFormRemoveMember({isOpenFormRemoveMember: props.isCloseFormRemoveMember});
    } 
        ...
        try {
          const transLeader = await groupAPI.swapRoomMaster({
            ...
            if (transLeader.status === 200) {
                toast.success("Chuyển nhóm trưởng thành công", {position: toast.POSITION.TOP_RIGHT, autoClose: 2000})
        ...
        {idLogin === props.master && (
          ...
          <div className={classes.Memberleft}>
            ...
            <div className={classes.nameMember}>
              <p>{user?.name}</p>
              <p className={classes.master}>{user?._id === props.master ? "Trưởng Nhóm" : ""}</p>
            </div>
          </div>
              ...
              <ul>
                <li>
                  ...
                  <ul className={classes.dropdown}>
                    <li onClick={openTransLeaderHandler}> <a>Chuyển nhóm trưởng</a></li>
                    <li onClick={openFormRemoveMemberHandler}> <a>Mời ra khỏi nhóm</a></li>
                  </ul>
                </li>
              </ul>
            </nav>
